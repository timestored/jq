...
public class JQLauncher implements QueryHandler {
	...
	private static final String QQ = ".Q.addr:-13!;.Q.a:lower .Q.A:\"ABCDEFGHIJKLMNOPQRSTUVWXYZ\";.Q.gc:-20!;.Q.hg:-200!;.Q.host:-12!;.Q.sha1:-33!;.Q.s1:-3!;.Q.s:-201!;.Q.n:\"0123456789\";.Q.qt:-202!;except:{[a;b]a where not a in b};inter:{[a;b] a where a in b};rank:{iasc iasc x}";
    ...
    public static void main(String [] args) throws Exception {
    	OptionParser cmdlParser = null;
    	OptionSet options = null;
    	if(args.length > 0) {
        	cmdlParser = JQParams.getParser();
        	options = cmdlParser.parse(args);
    	}
    	...
    	boolean quiet = options!=null && options.has("q");
        if(!quiet) {
        	System.out.println("     ██╗ ██████╗ ");
        	System.out.println("     ██║██╔═══██╗");
        	System.out.println("     ██║██║   ██║");
    	...
    	JQLauncher jql = new JQLauncher(quiet, options!=null && options.has("debug"), JQParams.getSystemCommands(options));
        String prompt = (options!=null && options.has("prompt")) ? (String) options.valueOf("prompt") : quiet ? "" : PROMPT;
    	Splitter.fixedLength(80).split(OpRegister.ops.keySet().toString()).forEach(System.out::println);
    	
        if(options!=null && options.has("eval")) {
    	...
    	if(options!=null && options.has("fancyconsole")) {
...
//	}
    ...
    public JQLauncher(boolean quiet, boolean debug, List<String> systemCommands) {
    	...
    	Object res = run(QQ);
    	helloRunner.setPreventBuiltinOverrides(true);
